...
class CarbonFitter():
    ...
    @partial(jit, static_argnums=(0,)) 
    def miyake_event(self, t, start_time, duration, phase, area):
    ...
    def run(self, time_values, params, y0):
        burn_in, _ = self.cbm.run(time_values, production=self.miyake_event, args=params, y0=y0)
        return burn_in
    ...
    def run_D_14_C_values(self, time_out, time_oversample, params, y0):
        d_14_c = self.cbm.run_D_14_C_values(time_out, time_oversample, 
                                       production=self.miyake_event, args=params, y0=y0, 
